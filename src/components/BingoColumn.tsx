...
export default function BingoColumn({ letter, columnNumbers, range }: Props) {
  ...
  return (
    <div className="flex flex-col">
      <div
        id={letter}
        className="flex items-center justify-center border bg-blue-800 text-xl font-extrabold text-white lg:text-5xl xl:h-14 xl:w-32"
      >
        {letter}
      </div>
      {columnNumbers.slice(minNumber, maxNumber).map((number) => (
        <div
          ...
          } h-6  w-20 flex-1 border border-white font-extrabold lg:text-5xl xl:w-32`}
        >
          {number.number}
        </div>
      ))}
