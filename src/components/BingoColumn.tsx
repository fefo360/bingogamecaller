...
export default function BingoColumn({ letter, columnNumbers, range }: Props) {
  ...
  return (
    <div className="flex flex-col">
      <div
        id={letter}
        className="flex items-center justify-center border bg-blue-800 text-xl lg:text-5xl font-extrabold text-white md:h-14"
      >
        {letter}
      </div>
      {columnNumbers.slice(minNumber, maxNumber).map((number) => (
        <div
          ...
          } h-6  w-20 flex-1 border border-white font-extrabold lg:text-5xl`}
        >
          {number.number}
        </div>
      ))}
